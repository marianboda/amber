...
import { ensureUnsortedTopic, topicsForBookmark, topicsForBookmarks } from "./topics.js";

const SAVED_FROM = new Set(["extension", "share_sheet", "context_menu", "import", "api"]);

// Split on any run of non-alphanumerics into prefix-matched tsquery lexemes,
// ANDed: `rust async` → `rust:* & async:*`, `unique-fts-token` →
// `unique:* & fts:* & token:*` (matching how the 'simple' tokenizer breaks the
// stored text). Splitting on punctuation also neutralizes tsquery operator
// syntax, so user input can't inject `!`/`|`/`(`.
export function ftsQuery(q: string): string | null {
  const terms = q.split(/[^\p{L}\p{N}_]+/u).filter(Boolean);
...
// ts_headline options for search snippets (mirrors the old FTS5 snippet()).
const HEADLINE_OPTS =
  "StartSel=<mark>, StopSel=</mark>, MaxFragments=1, MaxWords=18, MinWords=5, ShortWord=2";

export function bookmarkRoutes(db: Db, config: Config): Hono {
  const app = new Hono();
  ...
  app.post("/", async (c) => {
    ...
    try {
      ...
      // return the winner as a duplicate instead of a 500 (pg 23505 = unique).
      if (err?.code === "23505") {
    ...
    if (!deferEnrich) await enqueueJob(db, "enrich", { bookmark_id: id });
  ...
  app.get("/", async (c) => {
    ...
    // Clamp to [1, 200]; invalid/≤0 falls back to 50.
    ...
    // Search defaults to relevance ranking (title > gist/note > content_text)
    // with match snippets; sort=recent/oldest opts back into date order + cursor.
    const match = q ? ftsQuery(q) : null;
    const relevance = !!match && !oldest && sort !== "recent";
    if (q && match && !relevance) {
      where.push("b.fts @@ to_tsquery('simple', ?)");
      params.push(match);
    } else if (q && !match) {
      where.push("(b.title ILIKE ? OR b.gist ILIKE ? OR b.note ILIKE ?)");
      const like = `%${q}%`;
    ...
    // Stable cursor "savedAt.id": rows with an identical saved_at (common after
    // an import that shares one ADD_DATE) are not skipped. `before` pages
    // newest-first, `after` pages oldest-first (sort=oldest).
    ...
    if (cursor) {
      ...
      const cmp = oldest ? ">" : "<";
      if (cursorId) {
        where.push(`(b.saved_at ${cmp} ? OR (b.saved_at = ? AND b.id ${cmp} ?))`);
    ...
    // Everything except content_text/content_html/fts, which are large and only
    // needed by the detail view (GET /:id returns the full row).
    const cardColumns = `b.id, b.url, b.canonical_url, b.title, b.domain, b.favicon_url,
       b.og_image_url, b.saved_at, b.content_type, b.gist, b.summary, b.note, b.is_read,
       b.saved_from, b.device, b.referrer, b.source_detail, b.topic_hint, b.enrich_status,
       b.fetch_status, b.archive_ref, b.media_ref, b.media_status, b.title_locked, b.import_batch`;
    let rows: any[];
    if (relevance) {
      ...
      // The tsquery is bound once ($1) and reused via the q(query) alias.
      const sql = `SELECT ${cardColumns},
                     ts_headline('simple', coalesce(b.content_text, ''), q.query,
                       '${HEADLINE_OPTS}') AS snippet
                   FROM bookmarks b, to_tsquery('simple', ?) AS q(query)
                   WHERE b.fts @@ q.query ${where.length ? "AND " + where.join(" AND ") : ""}
                   ORDER BY ts_rank_cd(b.fts, q.query) DESC LIMIT ?`;
      rows = (await db.prepare(sql).all(match, ...params, max)) as any[];
    } else {
      const order = oldest ? "ASC" : "DESC";
      const sql = `SELECT ${cardColumns} FROM bookmarks b
                   ${where.length ? "WHERE " + where.join(" AND ") : ""}
                   ORDER BY b.saved_at ${order}, b.id ${order} LIMIT ?`;
      rows = (await db.prepare(sql).all(...params, max)) as any[];
    }
  ...
  app.get("/:id", async (c) => {
    ...
    delete row.fts; // don't ship the tsvector blob
    row.topics = await topicsForBookmark(db, row.id);
    return c.json(row);
  });
  ...
  app.patch("/:id", async (c) => {
    ...
    const exists = await db.prepare("SELECT id FROM bookmarks WHERE id = ?").get(id);
    ...
    if (Array.isArray(body.topics)) {
      const known = (await db.prepare("SELECT name FROM topics").all()) as { name: string }[];
    ...
    await db.tx(async (t) => {
      if (sets.length) {
        await t.prepare(`UPDATE bookmarks SET ${sets.join(", ")} WHERE id = ?`).run(...params, id);
      }
      if (Array.isArray(body.topics)) {
        // Topics already validated above; replace user-assigned set (by_ai=0).
        await t.prepare("DELETE FROM bookmark_topics WHERE bookmark_id = ?").run(id);
        for (const name of body.topics as string[]) {
          await t
            .prepare(
              `INSERT INTO bookmark_topics (bookmark_id, topic_id, by_ai)
               SELECT ?, id, 0 FROM topics WHERE name = ?`
            )
            .run(id, name);
        }
      }
    });
    const row = (await db.prepare("SELECT * FROM bookmarks WHERE id = ?").get(id)) as any;
    delete row.fts;
    row.topics = await topicsForBookmark(db, id);
    return c.json(row);
  });
  ...
  app.put("/:id/archive", async (c) => {
    const id = c.req.param("id");
    const row = (await db.prepare("SELECT id, archive_ref FROM bookmarks WHERE id = ?").get(id)) as
    ...
    const existingFile = row.archive_ref ? archivePath(config.dataDir, row.archive_ref) : null;
    if (existingFile && fs.existsSync(existingFile) && !replace) {
    ...
    await enqueueJob(db, "enrich", { bookmark_id: id }); // re-extract from the snapshot
    return c.json({ ok: true, bytes: html.length });
  });

  app.get("/:id/archive", async (c) => {
    const row = (await db
      .prepare("SELECT archive_ref FROM bookmarks WHERE id = ?")
      .get(c.req.param("id"))) as { archive_ref: string | null } | undefined;
  ...
  app.post("/enrich-missing", async (c) => {
    ...
    await db.tx(async (t) => {
      for (const { id } of rows) {
        await t.prepare("UPDATE bookmarks SET enrich_status = 'pending' WHERE id = ?").run(id);
        await enqueueJob(t, "enrich", { bookmark_id: id });
      }
    });
    const remaining = (
      (await db
  ...
  app.post("/:id/retry", async (c) => {
    const id = c.req.param("id");
    const row = (await db.prepare("SELECT enrich_status FROM bookmarks WHERE id = ?").get(id)) as
      | { enrich_status: string }
      | undefined;
    if (!row) return c.json({ error: "not found" }, 404);
    await db.prepare("UPDATE bookmarks SET enrich_status = 'pending' WHERE id = ?").run(id);
