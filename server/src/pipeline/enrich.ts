...
async function vocabulary(db: Db): Promise<string[]> {
  return ((await db.prepare("SELECT name FROM topics").all()) as { name: string }[]).map(
    (r) => r.name
  );
}
...
export async function applyTopics(db: Db, bookmarkId: string, names: string[]) {
  await db.tx(async (t) => {
    await t
      .prepare("DELETE FROM bookmark_topics WHERE bookmark_id = ? AND by_ai = 1")
      .run(bookmarkId);
    for (const name of names) {
      await t
        .prepare(
          `INSERT INTO bookmark_topics (bookmark_id, topic_id, by_ai)
           SELECT ?, id, 1 FROM topics WHERE name = ?
...
export async function enrichBookmark(
  ...
  const bookmark = (await db.prepare("SELECT * FROM bookmarks WHERE id = ?").get(bookmarkId)) as any;
  ...
  try {
    ...
    await db.prepare("UPDATE bookmarks SET enrich_status = 'failed' WHERE id = ?").run(bookmarkId);
    throw err;
  }
}
...
async function run(
  ...
  const topicNames = await vocabulary(db);
  ...
  if (isYouTube(bookmark.url)) {
    ...
    if (oembed) {
      ...
      await db.prepare("UPDATE bookmarks SET fetch_status = 'dead' WHERE id = ?").run(bookmarkId);
    }
    if (metadataOnly) {
      await db
        .prepare("UPDATE bookmarks SET content_type = 'video', enrich_status = 'done' WHERE id = ?")
        .run(bookmarkId);
        ...
        if (finalCanonical !== bookmark.canonical_url) {
          ...
          if (owner) {
            ...
            await db.tx(async (t) => {
                ...
                await t
                  .prepare(
                    `UPDATE bookmarks SET note = CASE
                       ...
                       ELSE note || chr(10) || chr(10) || ?
                     END WHERE id = ?`
                  )
                  .run(bookmark.note, bookmark.note, owner.id);
              }
              if (bookmark.is_read) {
                await t.prepare("UPDATE bookmarks SET is_read = 1 WHERE id = ?").run(owner.id);
              }
              if (bookmark.title) {
                await t
                  .prepare("UPDATE bookmarks SET title = COALESCE(NULLIF(title, ''), ?) WHERE id = ?")
              ...
              await t.prepare("DELETE FROM bookmarks WHERE id = ?").run(bookmarkId);
            });
            return;
          }
          await db
            .prepare("UPDATE bookmarks SET canonical_url = ?, domain = ? WHERE id = ?")
            .run(finalCanonical, domainOf(page.finalUrl), bookmarkId);
        }

        if (!page.isHtml) {
          // PDF/image/binary: no markup to extract or archive — keep the
          // bookmark, classify from title/URL alone, don't pollute FTS.
          await db.prepare("UPDATE bookmarks SET fetch_status = 'ok' WHERE id = ?").run(bookmarkId);
        } else {
    ...
    if (metadataOnly) {
      await db.prepare("UPDATE bookmarks SET enrich_status = 'done' WHERE id = ?").run(bookmarkId);
      await cacheAssets(db, config.dataDir, bookmarkId).catch(() => {});
      return;
    }
    checkAborted(signal);
  ...
  await applyTopics(db, bookmarkId, enrichment.topics);
