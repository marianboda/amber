...
function insertBookmark(fields: Record<string, unknown> = {}): string {
  const id = randomUUID();
  const row = {
    id,
    url: `https://m.test/${id}`,
  ...
  db.prepare(
    `INSERT INTO bookmarks (${cols.join(",")}) VALUES (${cols.map(() => "?").join(",")})`
  ).run(...Object.values(row));
  return id;
}

describe("maintenance rescue", () => {
  it("rescues an orphaned pending bookmark exactly once", () => {
    const id = insertBookmark();
    expect(runMaintenance(db)).toBe(1);
    const jobs = db
    ...
    expect(runMaintenance(db)).toBe(0);
    db.prepare("DELETE FROM bookmarks WHERE id = ?").run(id);
    db.prepare("DELETE FROM jobs WHERE bookmark_id = ?").run(id);
  });

  it("does not rescue a bookmark whose enrichment recently failed (no infinite loop)", () => {
    const id = insertBookmark();
    ...
    db.prepare("UPDATE jobs SET updated_at = ? WHERE bookmark_id = ?").run(
      now - FAILED_RESCUE_BACKOFF_SECONDS - 60,
      id
    );
    expect(runMaintenance(db)).toBe(1);
    db.prepare("DELETE FROM bookmarks WHERE id = ?").run(id);
...
describe("permanent failure stamps the bookmark", () => {
  it("marks the bookmark failed when its enrich job exhausts retries", async () => {
    ...
    stop();
    const row = db.prepare("SELECT enrich_status FROM bookmarks WHERE id = ?").get(id) as any;
...
describe("jobs.bookmark_id", () => {
  it("is populated by enqueueJob for enrich payloads", () => {
    const jobId = enqueueJob(db, "enrich", { bookmark_id: "bm-123" });
    const job = db.prepare("SELECT bookmark_id FROM jobs WHERE id = ?").get(jobId) as any;
...
describe("list payload", () => {
  it("excludes content_text from the list but keeps it on detail", async () => {
    ...
    db.prepare("UPDATE bookmarks SET content_text = ? WHERE id = ?").run(
      "x".repeat(5000),
      created.id
    );
...
describe("enrich-missing", () => {
  it("re-enqueues only done rows without a gist, respecting the limit", async () => {
    const missing1 = insertBookmark({ enrich_status: "done", gist: null });
    const missing2 = insertBookmark({ enrich_status: "done", gist: null });
    const enriched = insertBookmark({ enrich_status: "done", gist: "already has one" });
    const res = await (
      await app.request("/bookmarks/enrich-missing", json({ limit: 1 }))
    ).json();
    ...
    for (const id of [missing1, missing2]) {
      ...
      const jobs = db.prepare("SELECT id FROM jobs WHERE bookmark_id = ?").all(id);
      expect(jobs).toHaveLength(1);
    }
    const untouched = db
...
describe("import", () => {
  it("metadata mode tags enrich jobs and fills the progress column", async () => {
    ...
    const jobId = enqueueJob(db, "import", { filename: "meta.html", items, enrich: "metadata" });
    await runImport(db, { filename: "meta.html", items, enrich: "metadata" }, jobId);
    const job = db.prepare("SELECT payload, progress FROM jobs WHERE id = ?").get(jobId) as any;
    ...
    const enrichJobs = db
      .prepare("SELECT payload FROM jobs WHERE type = 'enrich' AND bookmark_id IN (SELECT id FROM bookmarks WHERE import_batch = ?)")
      .all(jobId) as any[];
  ...
  it("accepts enrich=metadata via query param", async () => {
    ...
    const job = db.prepare("SELECT payload FROM jobs WHERE id = ?").get(body.job_id) as any;
