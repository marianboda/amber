...
### 1.1 Fix infinite enrich-rescue loop `[x]` — S
...
### 1.3 List endpoint payload + N+1 + indexes `[x]` — S
...
### 2.1 Restore path for Amber's own export `[x]` — M–L
...
### 2.3 Graceful shutdown + DB hygiene `[x]` — S
...
### 2.4 Delete leaves orphan files `[x]` — S–M
...
### 3.1 Error surfacing everywhere `[x]` — S–M
...
### 3.3 Grid scalability `[x]` — S first, L if needed
...
### 3.4 URL routing / deep links `[x]` — M
...
### 3.5 Enrichment status polling `[x]` — M
...
### 3.6 Unsaved-note protection `[x]` — S
...
### 4.1 Non-HTML content handling `[x]` — S gate, M for PDF path
...
### 4.3 Search relevance + snippets `[x]` — M
...
### 4.4 Archive serving streams `[x]` — S
...
### 4.5 Raw-HTML fallback base URL `[x]` — S
...
### 5.1 `archive_coming` failure signal `[x]` — S
...
### 5.2 Capture timeout + size safety `[x]` — M
...
### 5.3 Save-page context menu + options nudge `[x]` — S
...
### 6.2 Enrichment pipeline tests `[x]` — M
...
### 7.3 Cache-Control hardening `[x]` — S
...
- [x] DEPLOY.md:22 — `git push dokku main` → `master`.
- [x] DEPLOY.md — add `AMBER_TRUST_PROXY=1` (required behind Dokku nginx per docs/STATUS.md:36) and nginx `client_max_body_size` note (300MB archive PUTs vs 1MB default).
- [x] DEPLOY.md:27 — replace raw-snapshot backup advice (unsafe under WAL) with 2.2's backup files.
- [x] TODO.md — stale: says 34 tests (46 real); lists reader mode and server-side archive fallback as open (both done).
- [x] Dockerfile — add `USER node`, `HEALTHCHECK`, root `.dockerignore` (exclude node_modules, `server/data/`, `.git`).
